(function() {
	...
	function inferDefaultServerUrl() {
		...
		const candidate = document.querySelector("script[src*=\"/poll-sdk.js\"]");
	...
	function init(initOptions) {
		if (!initOptions.installKey) throw new Error("BizmePoll init requires installKey");
		destroy();
	...
	function dispatch(command, initOptions) {
		if (command === "init") {
			if (!initOptions) throw new Error("BizmePoll init requires options");
			init(initOptions);
			return;
		}
		if (command === "destroy") {
	...
	const BizmePoll = (command, initOptions) => {
