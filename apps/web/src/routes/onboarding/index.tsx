...
function RouteComponent() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error) {
        setError(error.message ?? "Unable to create your workspace. Please try again.");
        return;
      }
    ...
    <main className="flex min-h-svh h-full w-full items-center justify-center bg-white bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] bg-size-[28px_28px] p-6">
      <Card className="w-full max-w-lg rounded-2xl">
        <CardHeader>
          ...
          <CardDescription>Let's start by setting up your workspace</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit}>
            <FieldGroup className="gap-3">
              <Field>
                ...
                <Input id="workspace-name" name="workspaceName" placeholder="Acme Inc." required />
              </Field>
              <Field>
              ...
              <Field>
                <Button type="submit" disabled={isPending}>
                  {isPending ? <LoadingDots color="currentColor" /> : "Continue"}
                </Button>
              </Field>
            </FieldGroup>
          </form>
