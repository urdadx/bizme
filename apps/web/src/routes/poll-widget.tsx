...
function PollWidgetRoute() {
  ...
  }, [apiUrl, pageUrl, search.hostColorScheme, search.installKey, search.pollId, visitorId]);
  ...
  async function vote(optionId: string) {
    ...
    try {
      const response = await fetchJson<EmbedPollResponse>(apiUrl, `/embed/polls/${poll.id}/vote`, {
        method: "POST",
        body: JSON.stringify({
          installKey: search.installKey,
          optionId,
  ...
  const isClosed =
    poll?.status !== "active" || (poll.closesAt ? new Date(poll.closesAt) <= new Date() : false);
  ...
  return (
    <div
      className={cn(
        "min-h-screen bg-background p-4 font-sans text-sm",
