...
const EMPTY_PAGES_DATA = [
  { id: "placeholder-1", pageName: "Homepage", url: "", comments: 42 },
  { id: "placeholder-2", pageName: "Pricing", url: "", comments: 31 },
  { id: "placeholder-3", pageName: "Docs", url: "", comments: 24 },
  { id: "placeholder-4", pageName: "Blog", url: "", comments: 18 },
  { id: "placeholder-5", pageName: "Contact", url: "", comments: 12 },
];
...
export function PagesOverview({ pagesData }: PagesOverviewProps) {
  ...
  const emptyPages = useMemo(() => {
    return EMPTY_PAGES_DATA.map((page) => ({
      icon: <GlobeLinear color="black" className="w-4 h-4" />,
      title: page.pageName,
      value: page.comments,
      href: page.url,
  ...
  const displayedPages = hasData ? topPages : emptyPages;
  const displayedMaxPageCount = hasData
    ? maxPageCount
    : Math.max(...emptyPages.map((page) => page.value), 0);
