...
export function CommentComposer({
  ...
  const [placeholder, setPlaceholder] = useState(animatedPlaceholders?.[0] ?? "Write a reply...");

  useEffect(() => {
    if (!animatedPlaceholders?.length) return;

    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      setPlaceholder(animatedPlaceholders[0] ?? "Write a reply...");
      return;
    }
    ...
    const animate = () => {
      ...
      if (!isDeleting) {
        ...
        if (characterIndex === phrase.length) {
          isDeleting = true;
          timeoutId = setTimeout(animate, 1400);
          return;
        }
      } else {
        characterIndex -= 1;
      ...
      timeoutId = setTimeout(animate, isDeleting ? 35 : 70);
    };
  ...
  return (
    ...
    >
      ...
      <PromptInputTextarea placeholder={placeholder} className="flex-1 text-foreground" />
        ...
        <PromptInputAction tooltip="Attach files">
          ...
          >
            <input
              ...
              id={uploadId}
            />
            <GalleryLinear color="#0288d1" className="size-5 text-primary" />
          </label>
        </PromptInputAction>
