  ...
  async function handleGoogleSignUp() {
    ...
    try {
      const { error } = await authClient.signIn.social({
        provider: "google",
        callbackURL: `${env.VITE_FRONTEND_ORIGIN}${redirectTo}`,
      });
    ...
    <form className={cn("flex flex-col gap-4", className)} onSubmit={handleSubmit} {...props}>
      <FieldGroup className="gap-4">
        ...
        <Field>
          ...
          <Input id="name" name="name" type="text" placeholder="Jack Maaye" required />
        </Field>
        <Field>
        ...
        <Field>
          <Button type="submit" disabled={isPending || isGooglePending}>
            {isPending ? <LoadingDots color="currentColor" /> : "Create my account"}
          </Button>
        </Field>
        ...
        <Field>
          ...
          >
            {isGooglePending ? <LoadingDots color="currentColor" /> : <GoogleSVG />}
            Sign up with Google
          </Button>
          <FieldDescription className="px-6 text-center">
            Already have an account?{" "}
            <Link to="/login" search={{ redirect: redirectTo, email: initialEmail }}>
              Sign in
            </Link>
          </FieldDescription>
        </Field>
