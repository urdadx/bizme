...
const options = [
  ...
  { label: "Vue", votes: 34, percent: 28, icon: VueIcon },
  { label: "Svelte", votes: 22, percent: 19, icon: SvelteIcon },
];

export function PollPreview({ customization }: { customization?: CustomizationSettingsValue }) {
  return (
    <div className="flex w-full max-w-xl flex-col gap-2">
      <div
        style={{ color: customization?.textColor }}
        className="h-fit pb-4 w-full z-0 rounded-xl border bg-white flex flex-col overflow-hidden text-sm text-zinc-900"
      >
              ...
              <div className="flex h-full flex-col">
                <div className="min-h-0 flex-1 px-4 pt-2">
                  ...
                  <div className="grid gap-2">
                    {options.map((option) => (
                      <div key={option.label} className="rounded transition-all hover:bg-blue-50">
                        <div className="group flex items-center justify-between px-1 py-1">
                          <div className="relative z-10 flex h-9 w-full min-w-0 max-w-[calc(100%-5rem)] items-center">
                            ...
                            <div
                              className="absolute h-full origin-left rounded-md opacity-25"
                              style={{
                                width: `${option.percent}%`,
                                backgroundColor: customization?.brandColor ?? "#bfdbfe",
                              }}
                            />
                          </div>
