...
const codeBlockClassName = cn(
	...
	"[&_code]:w-full",
	"[&_code]:grid",
	"[&_code]:overflow-x-auto smooth-div",
...
export const CodeBlockHeader = ({ className, ...props }: CodeBlockHeaderProps) => (
	<div
		className={cn("flex flex-row font-mono items-center border-b bg-secondary", className)}
		{...props}
	/>
);
		...
		<pre className="w-full text-xs">
			<code className="text-xs">
				{children
					?.toString()
					.split("\n")
					.reduce<Array<{ key: string; line: string }>>((items, line) => {
						const occurrence =
							items.filter((item) => item.line === line).length + 1;
						items.push({ key: `${line}-${occurrence}`, line });
						return items;
					}, [])
					.map(({ key, line }) => (
