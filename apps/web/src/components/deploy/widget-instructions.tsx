...
export const WidgetInstructions = ({ installKey, type = "widget", onClose }: WidgetInstructionsProps) => {
...
<?php });`
		: "";
	const selectedCode = type === "frameworks" ? reactCode : type === "wordpress" ? wordpressCode : embedCode;
	const selectedLanguage = type === "frameworks" ? "tsx" : type === "wordpress" ? "php" : "html";
	const selectedFilename = type === "frameworks" ? "BizmeComments.tsx" : type === "wordpress" ? "functions.php" : "widget.html";
	const selectedIcon = type === "frameworks" ? <ReactIcon className="size-5" /> : type === "wordpress" ? <WordPressIcon className="size-5 text-sky-600" /> : <WidgetIcon color="purple" />;
	const title = type === "frameworks" ? "Install with frameworks" : type === "wordpress" ? "Install on WordPress" : "Install the widget";
	const description = type === "frameworks"
	...
	const platformLabel = type === "frameworks" ? "React app" : type === "wordpress" ? "WordPress theme" : "Standard HTML website";
					...
					<div className="flex h-full min-h-0 min-w-0 flex-col overflow-y-auto p-8 scrollbar-hide">
						...
						<div className="pt-5 pb-2">
							...
							<p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
								{type === "frameworks" ? (
									"Choose the snippet for your framework and render the component once near the root of your app."
								) : type === "wordpress" ? (
						...
						{type === "wordpress" ? (
							<div className="mt-5 grid w-full min-w-0 overflow-hidden">
								<div className="mb-3">
									...
									<p className="text-xs text-gray-500 dark:text-gray-400">
										Create a file like <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-[10px] dark:bg-gray-800">bizme-comments.php</code>, upload it to <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-[10px] dark:bg-gray-800">wp-content/plugins</code>, then activate it in WordPress.
									</p>
								</div>
								<CodeBlock
									value="php"
